classdef FileReadTest < NfxTest
    ...
    methods (Test)
        ...
        function metadataEditsRemainIndependent(t)
            ...
            [file, ok, status] = nfx.File.read(filename); t.assertTrue(ok, status.message);
            ...
            [read, ok, status] = nfx.File.read(output); t.assertTrue(ok, status.message);
        ...
        function publicLimitsApplyBeforeAllocationAndAcrossImages(t)
            ...
            [file, ok, status] = nfx.File.read(filename, MaxBytes=count - 1);
            t.verifyFalse(ok); t.verifyEqual(status.code, 'ResourceLimit');
            t.verifyEmpty(file.images);
            [file, ok, status] = nfx.File.read(filename, MaxPixels=11);
            t.verifyFalse(ok); t.verifyEqual(status.code, 'ResourceLimit');
            t.verifyEmpty(file.images);
            [file, ok, status] = nfx.File.read(filename, MaxBytes=count, MaxPixels=12);
            t.assertTrue(ok, status.message); t.verifyNumElements(file.images, 2);
        end

        function invalidFileNamesUseStatus(t, invalidFilename)
            [file, ok, status] = nfx.File.read(invalidFilename);
            t.verifyFalse(ok); t.verifyEqual(status.code, 'InvalidInput');
            t.verifyEmpty(file.images);
        end

        function invalidLimitsUseStatus(t, invalidLimit)
            [~, ok, status] = nfx.File.read('unused.ntf', MaxBytes=invalidLimit);
            t.verifyFalse(ok); t.verifyEqual(status.code, 'InvalidInput');
            [~, ok, status] = nfx.File.read('unused.ntf', MaxPixels=invalidLimit);
            t.verifyFalse(ok); t.verifyEqual(status.code, 'InvalidInput');
        end
