...
These checks establish supported encoding and associations, not scientific sensor-model accuracy, covariance propagation, CSM execution or Level 2 conformance. Incomplete generic metadata is permitted. Wrapped GLAS/GFM models currently fail writing until the next milestone supplies effective frame/context resolution; multi-frame pixels are also part of that milestone. SNIP enforcement remains explicitly unavailable until the profile milestone.
