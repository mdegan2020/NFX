function observations = observeOpenJPEG(encoder)
    ...
    cases = {ramp,noise,rgb}; names = ["mono16_ramp" "msi16_noise" "rgb8_ramp"];
    rows = cell(1,6); next = 1;
    for k = 1:numel(cases)
        for profile = ["NPJE" "EPJE"]
            ...
            m = packed.metrics;
            rows{next} = table(names(k),profile,m.raw_bytes,m.codestream_bytes, ...
                m.encode_seconds,m.normalize_seconds,seconds,m.temporary_bytes,after-before, ...
                VariableNames={'Scene','Profile','RawBytes','CodestreamBytes', ...
                'EncodeSeconds','NormalizeSeconds','TotalSeconds','TemporaryBytes','MATLABDeltaBytes'});
            next = next+1;
            clear packed
        end
    end
