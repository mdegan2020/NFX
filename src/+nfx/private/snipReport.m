function report = snipReport(header,images,texts,records,des) %#codegen
    ...
    for k = 1:numel(images)
        ...
        h = images(k).header;
        report = snipIssue(report,images(k).number_frames ~= 1 || ~strcmp(h.pjust,'R') || ...
            ~any(h.nbpp == [8 16]) || ~any(strcmp(h.icords,{'G','D','N','S','U'})), ...
            'SNIPImageEncoding',sprintf('images(%d).header',k), ...
            'Supply a still unsigned 8/16-bit image, right-justified pixels and geographic corners.','Tables 9-1 and 10-5');
    end
